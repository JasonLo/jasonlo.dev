...
The most intuitive home, but also the most constrained. In the editor, latency is visible and blocking. If an agent takes minutes to refactor or traverse a large repository, the developer’s flow collapses. IDE agents must be fast, they are assistants, not batch processors. [**Example**](https://code.claude.com/)
